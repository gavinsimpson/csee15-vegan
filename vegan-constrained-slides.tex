...
\emph{Solution} scale one set of scores relative to the other
...
\begin{center}\includegraphics[width=0.7\linewidth]{vegan-constrained-slides_files/figure-beamer/triplot-2-1} \end{center}
...
@Blanchet2008 proposed a two-step solution for models where $R^2_{adj}$
makes sense
...
\begin{frame}{Constrained ordination worked example \textbar{} spring
meadow vegetation}

\begin{center}\includegraphics[width=0.7\linewidth]{vegan-constrained-slides_files/figure-beamer/meadows-cca-full-triplot-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{vegan-constrained-slides_files/figure-beamer/meadows-cca-reduced-triplot-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{vegan-constrained-slides_files/figure-beamer/meadows-rda-reduced-triplot-1} \end{center}
...
\textbf{vegan} has only recently caught up \& we're not there yet
...
We still haven't fully integrated \textbf{permute} into
\textbf{vegan}\ldots{}

\ldots{}but you can pass \textbf{vegan} functions a matrix of
permutations to use instead of having it generate its own
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{42}\NormalTok{)}
\NormalTok{perm <-}\StringTok{ }\KeywordTok{shuffleSet}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(env), }\DataTypeTok{nset =} \DecValTok{499}\NormalTok{, }\DataTypeTok{control =} \NormalTok{h)}
